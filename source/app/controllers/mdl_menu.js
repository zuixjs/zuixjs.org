...
function MdlMenu() {
  ...
  function onInit() {
    this.options().css = 'ul{ margin:0 !important; padding:0 !important; } li[disabled]{ pointer-events: none; }';
    /*
    [
      '@cdnjs/material-design-lite/1.3.0/material.indigo-pink.min.css',
  ...
  function onCreate() {
    ...
    const a = this.view('a');
    a.attr('id', menuId)
        .addClass('mdl-button mdl-button--raised mdl-js-button mdl-js-ripple-effect mdl-color-text--primary')
        .on('click', function() {
        ...
        if (a.attr('z-load')) {
          zuix.context(a, () => {
            ...
            a.find('.material-icons').css({
              ...
              marginTop: isMini ? '8px' : '16px',
              marginLeft: isMini ? '1px' : '2px',
              position: 'initial'
            });
          });
        }
